...
def place_postOnly(exchange: Exchange,
    ...
    status = 'new'
    while status != 'open':
        top_of_book = exchange.fetch_ticker(ticker)['bid' if side == 'buy' else 'ask']*1.001
        order = exchange.createOrder(ticker, 'limit', side, size/top_of_book, price=top_of_book,
...
def chase(exchange: Exchange,
    ...
    while order_status['status'] == 'open':
        ...
        since = datetime.now().timestamp() * 1000 - 10*amend_speed * 1000
...
def execute_spread(exchange: Exchange, buy_ticker: str, sell_ticker: str, size: int) -> None:
    ...
    order = chase(exchange,order,999)
    order = place_postOnly(exchange, second_ticker, second_side, size)
    order = chase(exchange,order)

    return exchange.fetch_orders(buy_ticker,nowtime),exchange.fetch_orders(sell_ticker,nowtime)
