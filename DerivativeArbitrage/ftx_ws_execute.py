...
class myFtx(ccxtpro.ftx):
    ...
    async def lifecycle_to_json(self,filename = 'Runtime/logs/ftx_ws_execute/latest_events.json'):
        ...
        shutil.copy2(filename, 'Runtime/logs/ftx_ws_execute/archive/'+datetime.utcfromtimestamp(self.exec_parameters['timestamp']/1000).replace(tzinfo=timezone.utc).strftime("%Y-%m-%d-%H-%M")+'_events.json')
    ...
    async def risk_reconciliation_to_json(self,filename = 'Runtime/logs/ftx_ws_execute/latest_risk_reconciliations.json'):
        ...
        shutil.copy2(filename, 'Runtime/logs/ftx_ws_execute/archive/'+datetime.utcfromtimestamp(self.exec_parameters['timestamp']/1000).replace(tzinfo=timezone.utc).strftime("%Y-%m-%d-%H-%M")+'_risk_reconciliations.json')
    ...
    async def build_state(self, weights,parameters): # cut in 10):
        ...
        with open('Runtime/logs/ftx_ws_execute/latest_request.json', 'w') as file:
            json.dump({symbol:data
                      ...
                      file,
                      cls=NpEncoder)
        shutil.copy2('Runtime/logs/ftx_ws_execute/latest_request.json','Runtime/logs/ftx_ws_execute/archive/' + datetime.utcfromtimestamp(self.exec_parameters['timestamp'] / 1000).replace(tzinfo=timezone.utc).strftime(
    ...
    async def update_exec_parameters(self): # cut in 10):
        ...
        for coin,coin_data in trades_history_dict.items():
            ...
            for symbol, data in coin_data.items():
                ...
                self.exec_parameters[coin][symbol]['aggressive_edit_price_depth'] = self.parameters['aggressive_edit_price_tolerance'] * \
                                                                                    self.exec_parameters[coin][symbol]['priceIncrement']
    ...
    def quoter(self, symbol, orderbook):
        ...
        if marginal_risk>0:
            ...
            # mkt order (implemented as ioc limit through the mkt) if target reached.
            ...
            # limit order if level is acceptable
                ...
                and np.abs(self.total_delta) > self.limit.limit:
    ...
    async def create_order(self, symbol, type, side, amount, price=None, params={}):
        ...
        if self.pending_new_histories(coin) != []:#TODO: rather incorporate orders_pending_new in risk, rather than block
            if self.pending_new_histories(coin,symbol) != []:
                ...
                # this happends mostly btw pending_new and create_order on the other leg. not a big deal...
            ...
            # set pending_new -> send rest -> if success, leave pending_new and give id. Pls note it may have been caught by monitor_order by then.
            ...
            try:
                ...
                if isinstance(e,ccxt.InsufficientFunds):
                    ...
                    self.myLogger.warning(f'create {clientOrderId} failed: {str(e)}')
            else:
        ...
        for coin, balance in balances.items():
            if coin in self.currencies.keys() \
                ...
                if size > 0:
                    ...
                    request = {'fromCoin':'USD','toCoin':coin,'size':np.abs(size)*mid}
                coros += [self.private_post_otc_quotes(request)]

        quoteId_list = await asyncio.gather(*coros)
        await asyncio.gather(*[self.private_post_otc_quotes_quote_id_accept({'quoteId': int(quoteId['result']['quoteId'])})
...
async def ftx_ws_spread_main_wrapper(*argv,**kwargs):
    ...
    while not allDone:
        try:
            #TODO today: rather read complex order
