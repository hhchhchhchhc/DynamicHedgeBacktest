...
async def mkt_speed(exchange, symbol, target_depth=10000):
    ...
    nowtime = nowtime + timedelta(microseconds=int(0.5*(nowtime.microsecond-datetime.now(timezone.utc).microsecond))) ### to have unbiased ref
...
async def fetch_futures(exchange,includeExpired=False,includeIndex=False,params={}):
    ...
    for i in range(0, len(markets)):
        ...
        if exchange.safe_string(market,'type') == 'future':
            future_carry = calc_basis(mark, market['index'], expiryTime, datetime.now(timezone.utc))
