...
def md_pq_to_pandas(data_directory: str, trade_date: datetime.datetime, instrument_id: int):
    pq_filename_prefix = os.path.join(data_directory, trade_date.strftime("%Y%m%d") + "_" + str(instrument_id) + "_")
    trade_filename = pq_filename_prefix + "trade.parquet"
    return pq.read_table(trade_filename).to_pandas()
...
trades = trades.rename(columns={"timestamp": "timestamp_epoch", "datetime": "datetime", "price": "price", "qty": "size",
                                "buyer_is_market_maker": "given"})
...
trades["bucket"] = trades["imbalance"].apply(lambda bucket: int(10 * bucket))
...
stats = trades[["bucket", "move_dpm"]].groupby("bucket").agg(
    {"move_dpm": [np.len, np.mean, np.std, q1, q5, q10, q20, q30, q40, q50, q60, q70, q80, q90, q95, q99]})
