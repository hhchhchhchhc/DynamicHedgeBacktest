...
class Backtest:
    def __init__(self, random_start=False) -> None:
        ...
        self.minimum_basis_spread = self._compute_minimum_basis_spread_on_whole_day()
        if random_start:
            first_time = self.spot_trades['exchange_timestamp_nanos'].iloc[0]
            last_time = self.spot_trades['exchange_timestamp_nanos'].iloc[-1]
            random_start_time = first_time + max(60 * 60 * 1000000000, random.randrange(last_time - first_time))
            self.minimum_basis_spread = self._compute_minimum_basis_spread_at_time(random_start_time)
        ...
        self.spot_market_bid_timestamp = None
        self.spot_market_ask_timestamp = None
        self.future_market_bid_timestamp = None
        self.future_market_ask_timestamp = None
        self.spot_bid = None
        self.spot_ask = None
        self.future_bid = None
        self.future_ask = None
        self.current_event_timestamp = None
        self.spot_bid_timestamp = None
        self.spot_ask_timestamp = None
        self.future_bid_timestamp = None
        self.future_ask_timestamp = None
    ...
    def run(self):
        ...
        while all([self.spot_tobs_index < spot_tobs_n, self.spot_trades_index < spot_trades_n,
            ...
            self.current_event_timestamp = first_timestamp
    ...
    def _spot_trade_event(self) -> None:
        ...
        if self.spot_bid is not None and spot_trade.buyer_is_market_maker and spot_trade.price <= self.spot_bid:
            self._passive_spot_buy()
        if self.spot_ask is not None and not spot_trade.buyer_is_market_maker and spot_trade.price >= self.spot_ask:
    ...
    def _future_trade_event(self) -> None:
        ...
        if self.future_bid is not None and future_trade.buyer_is_market_maker \
                and future_trade.price <= self.future_bid:
            self._passive_future_buy()
        if self.future_ask is not None and not future_trade.buyer_is_market_maker \
                and future_trade.price >= self.future_ask:
    ...
    def _spot_tob_event(self) -> None:
        ...
        self.spot_market_bid_timestamp = self.current_event_timestamp
        self.spot_market_ask = spot_tob.ask_price
        self.spot_market_ask_timestamp = self.current_event_timestamp
    ...
    def _future_tob_event(self) -> None:
        ...
        self.future_market_bid_timestamp = self.current_event_timestamp
        self.future_market_ask = future_tob.ask_price
        self.future_market_ask_timestamp = self.current_event_timestamp
        ...
        if (len(self.spot_vwap_buffer) >= self.vwap_horizon) and (len(self.future_vwap_buffer) >= self.vwap_horizon):
            ...
            if delta == 0 and enough_basis_spread:
                if spot_autocovariance < 0:
                    ...
                    self.spot_bid_timestamp = self.current_event_timestamp
                if future_autocovariance < 0:
                    self.future_ask = self.future_market_ask
                    self.future_ask_timestamp = self.current_event_timestamp
    ...
    def _compute_minimum_basis_spread_on_whole_day(self) -> float:
        spread = pd.merge(self.spot_tobs[['exchange_timestamp_nanos', 'bid_price']],
                          self.future_tobs[['exchange_timestamp_nanos', 'ask_price']], how='outer',
        ...
        spread['spread'] = 2 * (spread['ask_price'] - spread['bid_price']) / (spread['ask_price'] +
                                                                              spread['bid_price'])
    ...
    def _compute_minimum_basis_spread_at_time(self, random_start_time: int) -> float:
        spot = self.spot_tobs[self.spot_tobs['exchange_timestamp_nanos'].between(
            random_start_time - 60 * 60 * 1000000000, random_start_time)]
        future = self.future_tobs[self.future_tobs.exchange_timestamp_nanos.between(
            random_start_time - 60 * 60 * 1000000000, random_start_time)]
